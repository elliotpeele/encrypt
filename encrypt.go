...
func main() {
	em, err := NewEncryptionManager("/Users/elliot/backup_test/tmp/")
	if err != nil {
		panic(err)
	}

	// Create keyrings if they don't exist.
	if err := em.GenKey(); err != nil {
		panic(err)
	}

	buf := new(bytes.Buffer)
	w, err := em.Encrypt(buf)
	if err != nil {
		panic(err)
	}

	// Encrypt a message into a buffer.
	w.Write([]byte("foo bar"))
	if err := w.Close(); err != nil {
		panic(err)
	}

	// Decrypt the message from the buffer. Note that this re-reads the
	// keys from disk.
	r, err := em.Decrypt(buf)
	if err != nil {
		panic(err)
	}
